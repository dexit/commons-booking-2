 ...
 */
require_once( CB2_PLUGIN_ROOT . 'framework/includes/lib-temp/CMB2-field-Icon/cmb-field-icon.php' );
require_once( CB2_PLUGIN_ROOT . 'framework/includes/lib-temp/CMB2-field-Calendar/cmb-field-calendar.php' );
require_once( CB2_PLUGIN_ROOT . 'framework/includes/lib-temp/CMB2-field-Paragraph/cmb-field-paragraph.php' );
...
function cb2_admin_pages() {
	...
	$basic_interface = array(
		'cb2-holidays'      => array(
			...
			'count'         => "select count(*) from {$wpdb->prefix}cb2_global_period_groups where period_status_type_id = 6",
		),
		'cb2-items'         => array(
			'page_title' => 'Items',
		...
		'cb2-repairs'       => array(
			...
			'count'         => "select count(*) from {$wpdb->prefix}cb2_timeframe_user_period_groups where period_status_type_id = 5",
			'count_class'   => 'warning',
		),
		'cb2-locations'     => array(
		...
		'cb2-opening-hours' => array(
			...
			'count'         => "select count(*) from {$wpdb->prefix}cb2_location_period_groups where period_status_type_id = 4",
		),
		'cb2-timeframes'    => array(
			...
			'count'         => "select count(*) from {$wpdb->prefix}cb2_timeframe_period_groups where period_status_type_id = 1",
		),
		'cb2-bookings'    => array(
			'indent'      => 1,
...
function cb2_admin_init_menus() {
  ...
  add_menu_page( 'CB2', "CB2$notifications_string", $capability_default, 'cb2', 'cb2_options_page', 'dashicons-video-alt' );
	...
	foreach ( cb2_admin_pages() as $menu_slug => $details ) {
		$parent_slug  = ( isset( $details['parent_slug'] )  ? $details['parent_slug']  : 'cb2' );
